...
class ReopenedEvent extends BaseNotification
{
    ...
    public function html()
    {
        if($this->source->content->container instanceof Space) {
            return Yii::t('CalendarModule.notifications_views_CanceledEvent', '{displayName} reopened the event "{contentTitle}" in the space {spaceName}.', [
                'displayName' => Html::tag('strong', Html::encode($this->originator->displayName)),
                'contentTitle' => $this->getContentInfo($this->source, false),
                'spaceName' =>  Html::encode($this->source->content->container->displayName)
            ]);
        } else {
            return Yii::t('ContentModule.notifications_views_ContentCreated', '{displayName} reopened the event "{contentTitle}".', [
                'displayName' => Html::tag('strong', Html::encode($this->originator->displayName)),
                'contentTitle' => $this->getContentInfo($this->source, false)
            ]);
        }
